...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//CONSULTAS SQL PARA ADMINISTRADOR
//Consultas SQL
ligaCtrl.getSQL = async (req, res, next) => {
    connection.query(req.params.sql, (err, result) => {
    res.send(result);
})
};
...
//PARAMETROS
//Consulta Parametros
ligaCtrl.getParametro = async (req, res, next) => {
    connection.query('SELECT * FROM parametros', (err, result) => {
    res.send(result);
})
};
...
ligaCtrl.getDivisionales = async (req, res, next) => {
    connection.query('SELECT divisional.*,modalidad.modalidadnom, zona.zonanom FROM divisional,modalidad, zona where divisional.zonaid = zona.zonaid and divisional.modalidadid = modalidad.modalidadid and divisional.divisionalnom <> "Divisional Clasificatorio" order by divisionalid', (err, result) => {
    res.send(result);
...
//Consulta todas las divisionales
ligaCtrl.getDivisionalesCam = async (req, res, next) => {
...
ligaCtrl.getDivisional = async (req, res, next) => {
    connection.query('SELECT * FROM divisional where divisionalid = '+ req.params.id, (err, result) => {
    res.send(result);
    })
};
...
//Consulta datos y resultados equipos de un campeonato
ligaCtrl.getcanequiposcamtotal = async (req, res, next) => {
    connection.query('select * from (SELECT equipo.equipoid eid, equipo.equiponom enom, equipo.equipoesc, camequ.camequpto ptos,' +      
        '(select count(*) from partidos where campeonatoid =' + req.params.cam + ' and (equipoid1 = eid or equipoid2 = eid) and partidos.PartidosIng = 1) pj,' +
        '(select count(*) from partidos where  partidos.PartidosIng = 1 and campeonatoid =' + req.params.cam + ' and ((equipoid1 = eid and Equipo1Res > Equipo2Res) or  (equipoid2 = eid and Equipo2Res > Equipo1Res ))) pg,' +
        '(select count(*) from partidos where partidos.PartidosIng = 1 and campeonatoid =' + req.params.cam + ' and ((equipoid1 = eid and Equipo1Res < Equipo2Res) or  (equipoid2 = eid and Equipo2Res < Equipo1Res ))) pp,' +
        '(select count(*) from partidos where partidos.PartidosIng = 1 and campeonatoid =' + req.params.cam + ' and ((equipoid1 = eid and Equipo1Res = Equipo2Res) or  (equipoid2 = eid and Equipo2Res = Equipo1Res ))) pe,' +
        '(select ((select COALESCE(sum(Equipo1Res),0) from partidos where partidos.PartidosIng = 1 and campeonatoid =' + req.params.cam + ' and equipoid1 = eid) + ' +
        '(select COALESCE(sum(Equipo2Res),0) from partidos where partidos.PartidosIng = 1 and campeonatoid =' + req.params.cam + ' and equipoid2 = eid))) gf,' +
        '(select ((select COALESCE(sum(Equipo1Res),0) from partidos where partidos.PartidosIng = 1 and campeonatoid =' + req.params.cam + ' and equipoid2 = eid) + ' +
        '(select COALESCE(sum(Equipo2Res),0) from partidos where partidos.PartidosIng = 1 and campeonatoid =' + req.params.cam + ' and equipoid1 = eid))) gc,' +
        '(select gf - gc) difgol ' +
        ' FROM equipo, camequ WHERE camequ.equipoid = equipo.equipoid and equipo.equipoest = "Activo" and camequ.campeonatoid = ' + req.params.cam + 
        ') datos order by ptos desc, difgol desc, gf desc, gc desc;', (err, result) => {
        res.send(result);
})
};

//Consulta datos y resultados equipos del campeonato CLASIFICATORIO
ligaCtrl.getcanequiposcamtotalCLA = async (req, res, next) => {
    connection.query('select * from (SELECT equipo.equipoid eid, equipo.equiponom enom, equipo.equipoesc, camequ.camequpto ptos, ' +      
        '(select count(*) from partidos where campeonatoid =' + req.params.cam + ' and (equipoid1 = eid or equipoid2 = eid) and partidos.PartidosIng = 1) pj,' +
        '(select count(*) from partidos where  partidos.PartidosIng = 1 and campeonatoid =' + req.params.cam + ' and ((equipoid1 = eid and Equipo1Res > Equipo2Res) or  (equipoid2 = eid and Equipo2Res > Equipo1Res ))) pg,' +
        '(select count(*) from partidos where partidos.PartidosIng = 1 and campeonatoid =' + req.params.cam + ' and ((equipoid1 = eid and Equipo1Res < Equipo2Res) or  (equipoid2 = eid and Equipo2Res < Equipo1Res ))) pp,' +
        ...
        '(select COALESCE(sum(Equipo2Res),0) from partidos where partidos.PartidosIng = 1 and campeonatoid =' + req.params.cam + ' and equipoid1 = eid))) gc,' +
        '(select gf - gc) difgol, ' +
        '(select PartidosNomJug from partidos where (equipoid1 = eid or equipoid2 = eid) and PartidosNomJug like "%GRUPO%" LIMIT 1) Fecha ' +
        ' FROM equipo, camequ WHERE camequ.equipoid = equipo.equipoid and camequ.campeonatoid = ' + req.params.cam + 
        ') datos order by ptos desc, difgol desc, gf desc, gc desc;', (err, result) => {
        res.send(result);
})
};
...
ligaCtrl.createequipo = async (req, res, next) => {
    if(req.body.equipofot === '' && req.body.equipoesc === ''){
        connection.query('INSERT INTO equipo (equiponom, equipoobs, equipocanjug, equipotelcon, equipomailcon, equipofecins, equipoest) values("' + 
        req.body.equiponom + '","'  + req.body.equipoobs + '",' + req.body.equipocanjug + ',"' + 
        req.body.equipotelcon + '","' + req.body.equipomailcon + '","' + req.body.equipofecins + 
        '","Activo")', (err, result) => {
            console.log(err); 
            res.send(result);
        });
...
ligaCtrl.createequipoimg = async (req, res, next) => {
    ...
    connection.query('INSERT INTO equipoimg (equipoid, equipofot) values(' + 
    req.body.equipoid + ',"'  + req.body.equipofot + '")', (err, result) => {
        console.log(err); 
        res.send(result);
...
ligaCtrl.editequipo = async (req, res, next) => {
    var efot = req.body.equipofot;
    var eesc = req.body.equipoesc;

    if((typeof efot === 'undefined') && (typeof eesc === 'undefined')){
        ...
        '", equipoest = "'  + req.body.equipoest + '" WHERE equipoid = ' + req.body.equipoid, (err, result) => {
        res.send(result);
        connection.commit;
        })
    }else{
        if((typeof efot === 'undefined')  && (typeof eesc !== 'undefined')){
            connection.query('UPDATE equipo SET equiponom = "' + req.body.equiponom + 
            '", equipoobs = "' + req.body.equipoobs +'", equipocanjug = ' + req.body.equipocanjug + 
            ', equipotelcon = "' + req.body.equipotelcon + 
            '", equipomailcon = "' + req.body.equipomailcon + '", equipofecins = "' + req.body.equipofecins + 
            '", equipoesc = "' + eesc + '", equipoest = "'  + req.body.equipoest + '" WHERE equipoid = ' + req.body.equipoid, (err, result) => {
            res.send(result);
            connection.commit;
            }) 
        }else{
            if((typeof efot !== 'undefined')  && (typeof eesc === 'undefined')){   
                connection.query('UPDATE equipo SET equiponom = "' + req.body.equiponom + 
                '", equipoobs = "' + req.body.equipoobs + '", equipofot = "' + efot + 
...
ligaCtrl.getjugadores = async (req, res, next) => {
    connection.query('SELECT jugador.*, equipo.equiponom FROM jugador, equipo where jugador.equipoid = equipo.equipoid order by jugador.jugadorestado asc, equipo.equiponom, jugador.jugadornom', (err, result) => {
    res.send(result);
})
};
...
ligaCtrl.getjugadoreseq = async (req, res, next) => {
    connection.query('SELECT jugador.*, equipo.equiponom FROM jugador, equipo where jugador.equipoid = equipo.equipoid and jugador.equipoid = '+ req.params.eq + ' order by jugador.jugadorestado asc', (err, result) => {
        res.send(result);
    })
};
...
ligaCtrl.createjugador = async (req, res, next) => {
    ...
    if(req.body.jugadorfot === ''){
        connection.query('INSERT INTO jugador (jugadornom, equipoid, jugadorci, jugadoremail,jugadortel,jugadordir,jugadorfecnac,jugadorfecins,jugadornro,jugadorpos,jugadorestado) values("' + 
        ...
        req.body.jugadorpos + '", "Activo")', (err, result) => {
            console.log(err); 
...
//Suspendo jugador
ligaCtrl.editjugadorsus = async (req, res, next) => {
    connection.query('UPDATE jugador SET jugadorestado = "Suspendido" where jugadorid = ' + req.params.ju, (err, result) => {
        res.send(result);
        connection.commit;
})
};
...
//Habilito jugador
ligaCtrl.editjugadorhab = async (req, res, next) => {
    connection.query('UPDATE jugador SET jugadorestado = "Activo" where jugadorid = ' + req.params.ju, (err, result) => {
        res.send(result);
        connection.commit;
})
};
...
ligaCtrl.getpartidos = async (req, res, next) => {
    connection.query('select par.*, (select equipo.equiponom from equipo where equipo.equipoid = par.equipoid1) equiponom1, (select equipo.equiponom from equipo where equipo.equipoid = par.equipoid2) equiponom2 from partidos as par where par.campeonatoid =' + req.params.cam + ' order by par.PartidosFecJug desc', (err, result) => {
    res.send(result);
})
};
...
ligaCtrl.getpartidosequipo = async (req, res, next) => {
    ...
    "where p.PartidosIng = 1 and p.campeonatoid = campeonato.campeonatoid and (p.equipoid1 = " + req.params.id + " or p.equipoid2 = " + req.params.id + ")", (err, result) => {
    res.send(result);
    connection.commit;
})
};
...
ligaCtrl.getpartidosfeccam = async (req, res, next) => {
    console.log(req.params.cam);
    if((req.params.nom === 'COPA DE ORO') || (req.params.nom === 'COPA DE PLATA') || (req.params.nom === 'COPA DE BRONCE')){
        console.log('1');
        connection.query('select * from partidos where campeonatoid =' + req.params.cam + ' group by partidosNomJug  order by partidosNomJug', (err, result) => {
        res.send(result);
        })
    }else{
...
//Consulta las fechas de un partido por campeonato clasificatorio
ligaCtrl.getpartidosfecCla = async (req, res, next) => {
    connection.query('select par.*, (select equipo.equiponom from equipo where equipo.equipoid = par.equipoid1) equiponom1, ' +
    '(select equipo.equiponom from equipo where equipo.equipoid = par.equipoid2) equiponom2, ' + 
    '(select equipo.equipoesc from equipo where equipo.equipoid = par.equipoid1) equipoesc1, ' +
    '(select equipo.equipoesc from equipo where equipo.equipoid = par.equipoid2) equipoesc2 ' +
    ' FROM partidos as par WHERE par.campeonatoid =' + 
    req.params.cam + ' and PartidosNomJug like "%' + req.params.fec + '%" Order by PartidosNomCla, PartidosFec', (err, result) => {
        res.send(result);
})
};
...
ligaCtrl.getpartidosganados= async (req, res, next) => {
    ...
    req.params.equ + ' and Equipo2Res > Equipo1Res )) and partidos.PartidosIng = 1', (err, result) => {
    res.send(result);
})
};
...
ligaCtrl.getpartidosempatados= async (req, res, next) => {
    connection.query('select count(*) totemp from partidos where campeonatoid =' + 
    req.params.cam + ' and ((equipoid1 = ' +  req.params.equ + ' and Equipo1Res = Equipo2Res) or  (equipoid2 = ' +  
    req.params.equ + ' and Equipo2Res = Equipo1Res )) and partidos.PartidosIng = 1', (err, result) => {
    res.send(result);
})
...
ligaCtrl.getpartidosNomFec= async (req, res, next) => {
    connection.query('select distinct(PartidosNomJug) Partidos, (select campeonatoid from campeonato where campeonatonom = "GRUPOS" and campeonatoestado = "Activo" LIMIT 1) campeonatoid, (select campeonatonom from campeonato where campeonatonom = "GRUPOS" and campeonatoestado = "Activo" LIMIT 1) campeonatonom from ligadb.partidos pa, ligadb.campeonato ca where pa.campeonatoid = ca.campeonatoid and ca.campeonatoestado = "Activo" and pa.PartidosNomJug like "%' + req.params.nom + '%"  order by Partidos;', (err, result) => {
    res.send(result);
    })
};

//Consulta nombre de las fechas de los campeonatos CLASIFICATORIOS
ligaCtrl.getpartidosNomFecCla= async (req, res, next) => {
    connection.query('select distinct(pa.PartidosNomJug), pa.campeonatoid from ligadb.partidos pa, ligadb.campeonato ca where ca.divisionalid in(select divisionalid from ligadb.campeonato where campeonatonom = "' + req.params.camppal + '" and campeonatoestado = "Activo") and ca.campeonatoid = pa.campeonatoid and ca.campeonatonom = "' + req.params.cam + '";', (err, result) => {
    res.send(result);
    })
};
...
ligaCtrl.createpartido = async (req, res, next) => {
    connection.query('INSERT INTO partidos (campeonatoid, equipoid1, equipoid2, PartidosFec, PartidosHor, PartidosCan, Equipo1Res, Equipo2Res, PartidosObs, PartidosFecJug, PartidosNomJug,PartidosComplejo, PartidosNomCla, Equipo1Pun, Equipo2Pun, PartidosIng) values(' + 
    ...
    req.body.PartidosObs + '", ' +  req.body.PartidosFecJug + ', "' + req.body.PartidosNomJug + '", "' + req.body.PartidosComplejo + '", "' + req.body.PartidosNomCla 
    + '",0,0,0)', (err, result) => {
...
ligaCtrl.editpartido = async (req, res, next) => {
    ...
    ', PartidosNomJug = "' + req.body.PartidosNomJug + '", PartidosNomCla = "' + req.body.PartidosNomCla + '", PartidosComplejo = "' + req.body.PartidosComplejo + '" WHERE PartidosId = ' +
            ...
            if(r > 0) { 
                ...
                req.body.PartidosId, (err, result) => {
                    ...
                    if (err){
                        ...
                        connection.query('select sum(Equipo1Pun) as tot1 from partidos where campeonatoid = ' + req.body.campeonatoid  + ' and equipoid1 = '  + req.body.equipoid1, (err, result) => {
                            if(isNaN(parseInt(result[0].tot1))){
                                var tot1 = 0;
                            }else{
                                var tot1 = parseInt(result[0].tot1); 
                            }
                            connection.query('select sum(Equipo2Pun) as tot2 from partidos where campeonatoid = ' + req.body.campeonatoid  + ' and equipoid2 = '  + req.body.equipoid1, (err, result) => {
                                ...
                                var tot = tot1 + tot2;
                                connection.query('UPDATE camequ set camequpto = ' + parseInt(tot) + ' where campeonatoid = ' + req.body.campeonatoid  + ' and equipoid = ' + req.body.equipoid1, (err, result) => {
                                    console.log(err);
                                    connection.commit;
                                })
                            })   
                    ...
                    if (err){
                        ...
                        connection.query('select sum(Equipo1Pun) as tot1 from partidos where campeonatoid = ' + req.body.campeonatoid  + ' and equipoid1 = '  + req.body.equipoid2, (err, result) => {
                            if(isNaN(parseInt(result[0].tot1))){
                                var tot1 = 0;
                            }else{
                                var tot1 = parseInt(result[0].tot1); 
                            }
                            connection.query('select sum(Equipo2Pun) as tot2 from partidos where campeonatoid = ' + req.body.campeonatoid  + ' and equipoid2 = '  + req.body.equipoid2, (err, result) => {
                                if(isNaN(parseInt(result[0].tot2))){
                                    var tot2 = 0;
                                }else{
                                    var tot2 = parseInt(result[0].tot2); 
                                ...
                                //puntos totales de cada equipo
                                var tot = tot1 + tot2;
                                connection.query('UPDATE camequ set camequpto = ' + parseInt(tot) + ' where campeonatoid = ' + req.body.campeonatoid  + ' and equipoid = ' + req.body.equipoid2, (err, result) => {
                                    console.log(err);
                                    res.send(result);
                                    connection.commit;
                                })
...
ligaCtrl.deletepartido = async (req, res, next) => {
    connection.query('SELECT * FROM ligadb.partidos where PartidosId = ' + 
    req.params.par, (err, resu) => {
        //Vuelvo atras la suma de puntos para el equipo si ya se habia ingresado el resultado
        if(resu[0].PartidosIng === 1){
            var r = (parseInt(resu[0].Equipo1Res) - parseInt(resu[0].Equipo2Res)); 
            if(r > 0){ //Si ganó el equipo 1
                var eq1 = resu[0].equipoid1
                connection.query('UPDATE camequ set camequpto = camequpto - 3 where equipoid = ' + eq1, (err, result) => {
                    console.log(err);
                })
            }
            if(r < 0){ //Si ganó el equipo 2
                var eq2 = resu[0].equipoid2
                connection.query('UPDATE camequ set camequpto = camequpto - 3 where equipoid = ' + eq2, (err, result) => {
                    console.log(err);
                })
            }
            if(r === 0){ //Si empataron
                var eq1 = resu[0].equipoid1
                var eq2 = resu[0].equipoid2
                connection.query('UPDATE camequ set camequpto = camequpto - 1 where equipoid = ' + eq1, (err, result) => {
                    console.log(err);
                })
                connection.query('UPDATE camequ set camequpto = camequpto - 1 where equipoid = ' + eq2, (err, result) => {
                    console.log(err);
                })
            }
        }
...
ligaCtrl.getjugpars = async (req, res, next) => {
    ...
    req.params.par + ' and equipo.equipoid =' + req.params.eq + ' order by equipo.equiponom', (err, result) => {
    res.send(result);
})
};
...
ligaCtrl.getjugparsgol = async (req, res, next) => {
    ...
    req.params.cam + ' group by jugador.jugadorid HAVING SUM(jugpar.jugadorgol) > 0 order by jugadorgol desc', (err, result) => {
    res.send(result);
})
};
...
ligaCtrl.getjugparestadistica = async (req, res, next) => {
    connection.query('SELECT ju.jugadorid, count(ju.PartidosId) jugadorpar,  sum(ju.jugadormin) jugadormin,'+ 
    ' sum(ju.jugadorgol) jugadorgol, sum(ju.jugadorama) jugadorama,' +
    ' sum(ju.jugadorroj) jugadorroj, 0 jugadorsus' +
    ' FROM ligadb.jugpar ju, ligadb.partidos pa ' +
    ' where ju.PartidosId = pa.PartidosId and pa.PartidosIng = 1 and ju.jugadorid = ' +  req.params.ju + 
    ' group by ju.jugadorid ;' , (err, result) => {
        res.send(result);
    }) 
};
